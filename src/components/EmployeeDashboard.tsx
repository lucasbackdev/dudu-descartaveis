        ...
        {showCreate && (
          <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
            ...
            <div>
              ...
              {items.some(i => parseFloat(i.sale_price) > 0) && (
                <div className="mt-3 flex justify-between items-center bg-secondary rounded-full px-4 py-2">
                  <span className="text-sm font-semibold">Total</span>
                  <span className="text-base font-bold">
                    R$ {items.reduce((s, i) => s + (parseInt(i.quantity) || 0) * (parseFloat(i.sale_price) || 0), 0).toFixed(2)}
                  </span>
                </div>
              )}
            </div>
